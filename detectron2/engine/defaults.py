...
"""
    ...
    def test_continual_domain(cls, cfg, wandb=None):
        ...
        if 'coco' in dataset_name:
            # Use only available corruption types in the dataset
            test_domain = ['brightness', 'defocus_blur', 'elastic_transform'] if cfg.TEST.ADAPTATION.SELECT_DOMAIN else ['brightness', 'defocus_blur', 'elastic_transform'][:]
        else:       # kitti
        ...
        for d_idx, corrupt in enumerate(test_domain):
            ...
            # Print current domain being tested
            logger.info("="*80)
            logger.info(f"Testing on domain: {corrupt} ({d_idx+1}/{len(test_domain)})")
            logger.info("="*80)
        ...
        # adapt source domain
        # Print current domain being tested (source/original domain)
        logger.info("="*80)
        logger.info(f"Testing on domain: {dataset_name} (original/source domain)")
        logger.info("="*80)
        data_loader = cls.build_test_loader(cfg, dataset_name)
        if cfg.TEST.ADAPTATION.VALIDATION:
